...
impl Callbacks for Cb {
    fn send_char(&mut self, s: &str) {
        if let Ok(debug) = std::env::var("RECAD_DEBUG") {
...
impl Simulation {
    ...
    pub fn run(&self) -> Result<HashMap<String, HashMap<String, Vec<f64>>>, RecadError> {
        spdlog::debug!("run commands:\n{}", self.circuit.controls.join("\n"));
        let mut cb = Cb::new();
            ...
            for v in vecs {
                ...
                let data1 = match vals.data {
                    ...
                    ComplexSlice::Complex(list) => list
                        .iter()
                        .map(|f| (f.cx_real.powi(2) + f.cx_imag.powi(2)).sqrt()) // Magnitude
                        .collect(),
                };
    ...
    pub fn op(&mut self) -> Result<HashMap<String, Vec<f64>>, RecadError> {
        spdlog::debug!("run operating point:\n{}", self.circuit.controls.join("\n"));
        let mut cb = Cb::new();
    ...
    pub fn tran(
        ...
        stop: &str,
        start: &str,
    ) -> Result<HashMap<String, Vec<f64>>, RecadError> {
        spdlog::debug!(
