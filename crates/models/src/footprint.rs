use types::{
    error::RecadError,
    gr::{GraphicItem, Polyline, Pos, Pt},


};

//TODO this should not be in schema
...
impl TryFrom<String> for PadType {
    type Error = RecadError;
    fn try_from(s: String) -> Result<Self, Self::Error> {
        match s.as_str() {
            "thru_hole" => Ok(PadType::ThruHole),
            "smd" => Ok(PadType::Smd),
            "connect" => Ok(PadType::Connect),
            "np_thru_hole" => Ok(PadType::NpThruHole),
            _ => Err(RecadError::Pcb(format!("Invalid pad type: {}", s))),
        }
    }
}
...
impl TryFrom<String> for PadShape {
    type Error = RecadError;
    fn try_from(s: String) -> Result<Self, Self::Error> {
        match s.as_str() {
            ...
            "custom" => Ok(PadShape::Custom),
            _ => Err(RecadError::Pcb(format!("Invalid pad shape: {}", s))),
        }
    }
}
