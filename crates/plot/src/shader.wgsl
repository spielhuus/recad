...
struct InstanceInput {
    ...
    // Location 8 is skipped (padding)
    @location(8) color: vec4<f32>,
    @location(9) width: f32,
    @location(10) radius: f32,
    @location(11) type_id: u32,
...
struct VertexOutput {
    ...
    @location(3) dimensions: vec2<f32>, // Passed to fragment for stroke width calc
    @location(4) angles: vec2<f32>,     // Start Angle, Sweep Angle (for Arc)
};
...
fn vs_main(
    ...
    // Standard Quad Vertices (-0.5 to 0.5)
    ...
    if (instance.type_id == 5u) {
        // Expand quad to cover radius + stroke/2 + AA padding
        let outer_dist = instance.radius + (instance.width * 0.5) + 2.0;
        let size = outer_dist * 2.0;
        let final_pos = instance.p1 + (v_pos * size);
        
        out.clip_position = camera.view_proj * vec4<f32>(final_pos, 0.0, 1.0);
        
        // We pass the position relative to center in World Units as 'uv'
        // v_pos is -0.5..0.5, size is full diameter.
        out.uv = v_pos * size; 
        
        out.dimensions = vec2<f32>(instance.width, instance.radius);
        out.angles = instance.p2; // [Start, Sweep]
        return out;
    }

    // --- CASE 2: TEXT (Type 4) ---
    if (instance.type_id == 4u) {
       // p1 is Center. p2 is Size. angle is rotation in radians.
       ...
       // Because projection flips Y, local Y -0.5 is visually TOP in world space (smaller Y).
       // We want Texture V=0 at the TOP.
       let local_uv = vec2<f32>(v_pos.x + 0.5, v_pos.y + 0.5); 
       
       let uv_w = instance.width;
       let uv_h = instance.radius;
       out.uv = instance.p3 + (local_uv * vec2<f32>(uv_w, uv_h));
    ...
    if (instance.type_id == 0u) {
        ...
        // Scale Quad: X becomes Length, Y becomes Thickness
